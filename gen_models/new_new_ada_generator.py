    ...
    def get_loss(self, perm, vgg, target, dis=None, layers=[]):
        ...
        if vgg is not None:
            ...
            for layer in layers:
                ...
                l_per = 1 / loss.array / 10 if self.l_per < 0 or self.l_per > 1000 else self.l_per#l_per loss
...
class AdaBIGGAN(AdaGenerator):
    def __init__(self, config, batchsize, comm=None, test=False):
        ...
        if config.lr_g_linear > 0:
            params["g_linear"] = gen.G_linear
        params["gen"] = gen
        super(AdaBIGGAN, self).__init__(**params)
        if not test:
            self.setup_optimizer()
            self.z.W.update_rule.hyperparam.alpha = 0.02
...
class AdaSNGAN(AdaGenerator):
    def __init__(self, config, batchsize, comm=None, test=False):
        ...
        gen = SNGAN(n_classes=config.n_classes, normalize_stat=self.normalize_stat)
        gen.initialize_params()  # initialize gamma and beta
